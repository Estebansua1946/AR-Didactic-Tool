...
class OpenGLGlyphs(QMainWindow):
    ...
    def initOpengl(self, width, height, pos_x = 380, pos_y = 0, window_name = b'AR Didactic Tool'): #inicializacion de opengl con variables por predeterminadas
        ...
        for var in range(1, 50):#nmodels +1
    ...
    def draw_background(self, image): #dibujo del background (camara)
    ...
    def draw_objects(self, image, mark_size = 0.05): #dibujo de los modelos 3d sobre el marcador aruco
...
# definiciones Interfaz -------------------------------------------------------------------------------------
    ...
    def Opacity(self):
        if self.ui.ButtOpac.isChecked(): 
            glEnable(GL_BLEND)           
            #self.ui.ButtOpac.setEnabled(False)
        else:
            glDisable(GL_BLEND)
    ...
    def main(self):
        self.initOpengl(640, 480)#self.image_w, self.image_h
        glutMainLoop()
